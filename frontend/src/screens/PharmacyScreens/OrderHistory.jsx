...
const OrderHistory = () => {
  ...
  useEffect(() => {
    getOrders();
    if(ordersData[0] === null) toast.error("No orders found")
  }, [""])
  ...
  const getOrders = async (e) => {
      var myHeaders = new Headers();
        ...
        const res = await fetch(`${baseUrl}/pharmacy/acceptedOrders`, requestOptions);
                ...
                <Card sx={{ minWidth: 275, marginTop:'1rem', minHeight:'5rem'}} variant="outlined" key={index}>
                  <CardContent sx={{display:'grid', gridTemplateColumns:'2fr 1fr', textAlign:'center'}}>
                    {/* {console.log(order)} */}
                      ...
                      <Typography variant="body2" sx={{marginTop:'0.5rem'}}>
                        Order ID : {String(order.orderID).slice(26,)}
                      </Typography>
                      </div>
                  </CardContent>
                </Card>
